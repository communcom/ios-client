...
class ContentHuggingScrollView: UIScrollView {
    ...
    func commonInit() {
        ...
        if scrollableAxis == .vertical {
            contentView.widthAnchor.constraint(equalTo: widthAnchor, constant: -(contentInset.top + contentInset.bottom)).isActive = true
        } else {
            contentView.heightAnchor.constraint(equalTo: heightAnchor, constant: -(contentInset.left + contentInset.right)).isActive = true
        }
    }
}
