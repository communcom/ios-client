...
extension CMActionSheet.Action {
    ...
    static func iconFirst(title: String, textWeight: UIFont.Weight = .medium, iconName: String, handle: (() -> Void)?, bottomMargin: CGFloat? = nil, showNextButton: Bool = false) -> CMActionSheet.Action {
        customLayout(height: 65, title: title, textSize: 17, textWeight: textWeight, spacing: 10, iconName: iconName, iconSize: 35, showIconFirst: true, showNextButton: showNextButton, bottomMargin: bottomMargin, handle: handle)
    }
    
    static func customLayout(height: CGFloat = 50, padding: UIEdgeInsets = UIEdgeInsets(top: 0, left: 16, bottom: 0, right: 16), title: String, textSize: CGFloat = 15, textWeight: UIFont.Weight = .medium, textColor: UIColor = .appBlackColor, spacing: CGFloat = 10, showIcon: Bool = true, iconName: String? = nil, iconSize: CGFloat = 24, iconTintColor: UIColor? = nil, showIconFirst: Bool = false, showNextButton: Bool = false, bottomMargin: CGFloat? = nil, handle: (() -> Void)?) -> CMActionSheet.Action {
        let stackView = UIStackView(axis: .horizontal, spacing: spacing, alignment: .center, distribution: .fill)
        let label = UILabel.with(text: title, textSize: textSize, weight: textWeight, textColor: textColor)
