...
extension ResponseAPIContentGetPost {
    var attachments: [ResponseAPIContentBlock] {
        let type = content.attributes?.type
        if type == "basic" {
            return content.content.arrayValue?.first(where: {$0.type == "attachments"})?.content.arrayValue ?? []
        }
        
        if type == "article" {
            return content.content.arrayValue?.filter {$0.type == "image" || $0.type == "video" || $0.type == "website"} ?? []
        }
    ...
    var firstEmbedImageURL: String? {
        ...
        if type == "article" {
            return content.content.arrayValue?.first(where: {$0.type == "image" || $0.type == "video" || $0.type == "website"})?.attributes?.thumbnail_url
        }
