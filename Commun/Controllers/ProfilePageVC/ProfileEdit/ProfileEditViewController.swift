...
class ProfileEditViewController: UIViewController {
    ...
    @IBOutlet weak var saveButton: UIButton! {
        didSet {
            self.saveButton.tune(withTitle: "save".localized().uppercaseFirst,
                                 textColor: .white,
                                 font: UIFont.systemFont(ofSize: .adaptive(width: 15), weight: .bold),
                                 alignment: .center)
        ...
        didSet {
            self.titleLabelsCollection.forEach {
                $0.tune(withText: $0.text!.localized().uppercaseFirst,
                        ...
                        font: UIFont.systemFont(ofSize: .adaptive(width: 12), weight: .semibold),
                        alignment: .left,
                        isMultiLines: false)
            }
        }
        ...
        didSet {
            self.textFieldsCollection.forEach {
                $0.tune(withPlaceholder: $0.placeholder!.localized().uppercaseFirst,
                        textColor: .black,
                        font: UIFont.systemFont(ofSize: .adaptive(width: 17), weight: .semibold),
                        alignment: .left)
    ...
    @IBOutlet weak var bioTextView: UITextView! {
        didSet {
            ...
            self.bioTextView.tune(with: biographyValue == nil ? .appMainColor : .black,
                                  font: UIFont.systemFont(ofSize: .adaptive(width: 17), weight: .semibold),
                                  alignment: .left)
